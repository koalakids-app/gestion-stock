// Edge function "dossier-pieces-employe"
// ============================================================================
// API publique (aucune session Supabase) pour la page pieces-employe.html, où
// un(e) salarié(e) dépose en ligne les documents demandés à l'embauche. Même
// principe que dossier-pieces / pieces.html côté famille, en parallèle : la
// page ne touche à aucune table directement, tout passe par cette fonction
// en service_role, qui ne renvoie/n'écrit que ce qui correspond au jeton
// reçu.
//
// Actions (POST, body JSON) :
//   {action:'get', token}
//     -> {dossier:{expire_le,statut}, employe:{prenom,creche_nom}, pieces:[{piece_key,filename,created_at}]}
//   {action:'upload', token, piece_key, filename, content_type, content_base64}
//     -> {ok:true}
//
// Stockage : bucket PRIVÉ `documents-employes` (même bucket que l'import fait
// par les référentes depuis la fiche collaborateur), table
// `employes_documents`. Les fichiers déposés par le/la salarié(e) ont
// piece_key renseigné et dossier_id pointant vers ce dossier ; ceux importés
// par une référente ont dossier_id=null.
//
// Taille max acceptée : 8 Mo par fichier (limite de charge utile raisonnable
// pour un scan/photo ; au-delà, la personne doit compresser ou photographier
// en plusieurs fois).
//
// Déploiement :
//   supabase functions deploy dossier-pieces-employe --no-verify-jwt
// (--no-verify-jwt : le/la salarié(e) n'a pas forcément de compte, voir
...
const BUCKET = "documents-employes";
const MAX_BYTES = 8 * 1024 * 1024;

// Pièces obligatoires (hors "le cas échéant") — RECOPIÉ de employes.html
// (constante PIECES_EMBAUCHE, entrées sans `optionnel:true`) : si vous
...
// deux versions ne se synchronisent pas (même choix assumé que pour
// PIECES_ADMIN entre demandes.html et pieces.html).
const REQUIRED_PIECE_KEYS = [
  "cv", "piece_identite", "carte_vitale", "justificatif_domicile", "diplomes",
  "certificat_medical", "attestation_honorabilite", "casier_judiciaire",
  "carnet_sante_vaccins", "rib",
];
...
Deno.serve(async (req) => {
  ...
  const { data: dossier, error: dErr } = await supabase
    ...
    .select("id, employe_id, statut, expire_le")
    .eq("token", token)
    .maybeSingle();
  if (dErr) {
    console.error("[dossier-pieces-employe] lecture dossier", dErr);
  ...
  if (action === "get") {
    ...
    const { data: pieces, error: pErr } = await supabase
      .from("employes_documents")
      .select("piece_key, filename, created_at")
      .eq("employe_id", dossier.employe_id)
      .not("piece_key", "is", null)
      .order("created_at", { ascending: false });
    if (pErr) {
      console.error("[dossier-pieces-employe] lecture pieces", pErr);
      return erreur("erreur_lecture", 500);
    }
    return json({
  ...
  if (action === "upload") {
    ...
    const path = `${dossier.employe_id}/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;

    const { error: upErr } = await supabase.storage.from(BUCKET).upload(path, bytes, { contentType });
    if (upErr) {
      console.error("[dossier-pieces-employe] upload storage", upErr);
      return erreur("upload_impossible", 500);
    }

    const { error: insErr } = await supabase.from("employes_documents").insert({
    ...
    if (insErr) {
      console.error("[dossier-pieces-employe] insertion", insErr);
